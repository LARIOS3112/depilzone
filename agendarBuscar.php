...
<tbody>
            ...
            if($archivo_seleccionado==''){
                $query = "select c.id_sede,c.id, c.fecha, c.hora,  DATE_FORMAT(c.fecha, '%d/%m/%Y') fecha_format, DATE_FORMAT(c.hora, '%h:%i %p') hora_format,
...
while($lista = mysqli_fetch_object($resultado)){
    ...
    <td>
        <a href="citas3<?php echo $archivo_seleccionado;?>.php?id_ci=<?php echo $id_c;?>&id_cli=<?php echo $id_cliente;?>&id_tratamiento=<?php echo $id_tto; ?>&procedencia=editar_cita" target="_blank" class="btn-hover3" title="Editar">
            <div class="fa-lg">
